    ...
    def load(self, config):
        ...
        if config is None:
            ...
            for i in range(1, 256):
                ...
                new_web = {'url': config.get_value(self._section, f'{postfix}url')}
                if new_web['url'] is None:
                    continue
                url_parse = urlparse(new_web['url'])
                if not bool(url_parse.scheme) or not bool(url_parse.netloc):
                    logger.error(
                        f"Bad URL ({new_web['url']}) in the [{self._section}] section of configuration file."
                    )
                    continue
                ...
                # RTT warning
                new_web['rtt_warning'] = config.get_value(
                ...
                # Indice
                new_web['indice'] = f'web_{str(i)}'

                # ssl_verify
                new_web['ssl_verify'] = config.get_value(
                    self._section, f'{postfix}ssl_verify', default=True
                )
                # Proxy
                http_proxy = config.get_value(
                    self._section, f'{postfix}http_proxy', default=None
                )

                https_proxy = config.get_value(
                    self._section, f'{postfix}https_proxy', default=None
                )
                ...
                logger.debug(f"Add Web URL {new_web['url']} to the static list")
