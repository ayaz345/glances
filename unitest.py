...
class TestGlances(unittest.TestCase):
    ...
    def test_001_plugins(self):
        ...
        print(
            f"INFO: [TEST_001] Check the mandatory plugins list: {', '.join(plugins_to_check)}"
        )
    ...
    def test_002_system(self):
        ...
        print(f"INFO: [TEST_002] Check SYSTEM stats: {', '.join(stats_to_check)}")
    ...
    def test_003_cpu(self):
        ...
        print(
            f"INFO: [TEST_003] Check mandatory CPU stats: {', '.join(stats_to_check)}"
        )
    ...
    def test_004_load(self):
        ...
        print(f"INFO: [TEST_004] Check LOAD stats: {', '.join(stats_to_check)}")
    ...
    def test_005_mem(self):
        ...
        print(f"INFO: [TEST_005] Check MEM stats: {', '.join(stats_to_check)}")
    ...
    def test_006_swap(self):
        ...
        print(f"INFO: [TEST_006] Check SWAP stats: {', '.join(stats_to_check)}")
        ...
        for stat in stats_to_check:
            ...
            # Check that % is > 0
            self.assertGreaterEqual(stats_grab[stat], 0)
        print(f'INFO: SWAP stats: {stats_grab}')
    ...
    def test_017_programs(self):
        ...
        print(f'INFO: PROGRAM list stats: {len(stats_grab)} items in the list')
        # Check if number of processes in the list equal counter
        # self.assertEqual(total, len(stats_grab))
    ...
    def test_095_methods(self):
        ...
        for plugin in plugins_list:
            for method in mandatories_methods:
                self.assertTrue(
                    hasattr(stats.get_plugin(plugin), method),
                    msg=f'{plugin} has no method {method}()',
                )
    ...
    def test_096_views(self):
        ...
        for plugin in plugins_list:
            ...
            self.assertTrue(type(views_grab) is dict, msg=f'{plugin} view is not a dict')
